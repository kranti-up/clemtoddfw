...
class MultiWOZDatabase:
    ...
    def _load_data(self, database_path: Text):
        ...
        for domain in self.DOMAINS:
            db_load_path = os.path.join(database_path, f"{domain}_db.json")
            logger.info(f"Loading MultiWOZ database for domain {domain}, path = {db_load_path}")
            ...
            if domain == 'taxi':
                #This is added as the JSON file contains array of dictionaries for taxi domain
                database_data[domain] = database_data[domain][0]
